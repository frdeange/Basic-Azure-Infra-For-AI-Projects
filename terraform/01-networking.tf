...
resource "azurerm_network_security_group" "apim_nsg" {
  ...
  # Only allows outbound traffic to the private endpoint of Azure OpenAI (Cognitive Services)
  # The IP is dynamically obtained from the private endpoint resource
  destination_address_prefix = azurerm_private_endpoint.pe_cognitive.private_service_connection[0].private_ip_address
  }
    # Allow inbound management endpoint (Azure portal, PowerShell)
    security_rule {
    ...
    # Allow outbound to Storage, Key Vault, SQL, AzureMonitor, etc.
    security_rule {
  ...
  # Outbound general 443 (needed for TLS/OCSP validation and miscellaneous endpoints not covered by specific service tags)
    security_rule {
    ...
    # Additional required outbound connectivity
    security_rule {
  ...
  # Health probe from Azure Load Balancer (useful for future scaling / higher SKUs)
    security_rule {
...
# Example UDR for APIM subnet to allow management endpoint return path
resource "azurerm_route_table" "apim_udr" {
  name                = "${var.ai_prefix}-rt-apim"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
# 01-networking.tf – Network Infrastructure
...
# Bastion subnet (must be named AzureBastionSubnet) - only if enabled
...
# Gateway subnet for VPN (if enabled) - must be named GatewaySubnet
